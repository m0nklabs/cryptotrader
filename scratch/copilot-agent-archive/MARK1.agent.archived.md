...
### 2026-02-09: Dependabot CI Unblock
**What happened:**
- Dependabot PRs were blocked by a workflow checkout token requirement (secrets unavailable on Dependabot PRs).
- Implemented conditional checkout steps to avoid passing an empty `token` input.

**Lessons for MARK1:**
- GitHub Actions reruns reuse the old workflow definition; to pick up a workflow fix, trigger a fresh run (e.g., a new commit) rather than only rerunning.
- If `gh pr edit` fails due to deprecated GraphQL fields, update PR metadata via `gh api` PATCH instead.

---

*MARK1 v1.5 - Updated 2026-02-09*
